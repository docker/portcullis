...
func TestContainsRecognisesKnownTokens(t *testing.T) {
	...
	cases := []struct {
		...
		{"etherscan_api_key", "ETHERSCAN_API_KEY=" + strings.Repeat("A", 34)},
		{"moralis_api_key", "moralis_api_key=" + strings.Repeat("a", 64)},
		...
		// Ninth batch — patterns cross-checked against the
		// TruffleHog v3 detector catalogue (`pkg/detectors/`).
		{"pinecone_canonical", "pcsk_" + strings.Repeat("a", 6) + "_" + strings.Repeat("b", 63)},
		{"langsmith_personal_token", "lsv2_pt_" + strings.Repeat("a", 32) + "_" + strings.Repeat("b", 10)},
		{"langsmith_service_key", "lsv2_sk_" + strings.Repeat("a", 32) + "_" + strings.Repeat("b", 10)},
		{"nvidia_nim_api_key", "nvapi-" + strings.Repeat("a", 64)},
		{"nightfall_api_key", "NF-" + strings.Repeat("a", 32)},
		{"zapier_webhook", "https://hooks.zapier.com/hooks/catch/" + strings.Repeat("a", 16)},
		{"tines_webhook", "https://my-tenant.tines.com/webhook/" + strings.Repeat("a", 32) + "/" + strings.Repeat("b", 32)},
		{"pubnub_publish_key", "pub-c-12345678-1234-1234-1234-123456789abc"},
		{"pubnub_subscription_key", "sub-c-12345678-1234-1234-1234-123456789abc"},
		{"rootly_api_token", "rootly_" + strings.Repeat("f", 64)},
		{"voiceflow_api_key", "VF.DM." + strings.Repeat("a", 24) + "." + strings.Repeat("b", 16)},
		{"voiceflow_api_key_no_channel", "VF." + strings.Repeat("a", 24) + "." + strings.Repeat("b", 16)},
		{"deno_personal_token", "ddp_" + strings.Repeat("a", 36)},
		{"deno_write_token", "ddw_" + strings.Repeat("a", 36)},
		{"ubidots_token", "BBFF-" + strings.Repeat("a", 30)},
		{"circleci_pat_v2", "CCIPAT_" + strings.Repeat("a", 22) + "_" + strings.Repeat("f", 40)},
		{"endorlabs_api_token", "endr+" + strings.Repeat("a", 16)},
		{"bitbucket_app_password", "ATBB" + strings.Repeat("a", 60)},
		{"stripe_payment_intent_client_secret", "pi_" + strings.Repeat("a", 24) + "_secret_" + strings.Repeat("b", 25)},
		{"trufflehog_enterprise_key", "thog-key-" + strings.Repeat("f", 16)},
		{"trufflehog_enterprise_secret", "thog-secret-" + strings.Repeat("f", 32)},
		{"ramp_api_id", "ramp_id_" + strings.Repeat("a", 40)},
		{"ramp_api_secret", "ramp_sec_" + strings.Repeat("a", 48)},
		{"intra42_secret", "s-s4t2ud-" + strings.Repeat("a", 64)},
		{"intra42_id", "u-s4t2af-" + strings.Repeat("a", 64)},
